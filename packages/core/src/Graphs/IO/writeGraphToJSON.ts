...
export const WriteVariablesToJSON = (
  registry: IRegistry,
...
export function writeGraphToJSON(
  ...
  // save custom events
  graphJson.customEvents = Object.values(graph.customEvents).map(
    (customEvent) => serializeCustomEvent(customEvent)
  );

  // save variables
  graphJson.variables = WriteVariablesToJSON(registry, graph.variables);
