...
import type { ProviderConfig } from '../src/abstractions/types.js';
...
describe('createModel', () => {
  it('uses the OpenAI provider for openai/openrouter/custom kinds', () => {
    for (const kind of ['openai', 'openrouter', 'custom'] as const) {
      const model = createModel(config({ kind }), 'gpt-4o-mini');
      expect(model).toBeTypeOf('object');
      const { modelId, provider } = model as {
        modelId: string;
        provider: string;
      };
...
describe('resolveApiKey', () => {
  const credentials = {
    getApiKey: (ref: string) =>
      ({ openai: 'sk-by-kind', 'my-key': 'sk-by-ref' })[ref]
  };
